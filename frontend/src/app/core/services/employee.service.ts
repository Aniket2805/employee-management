...
export class EmployeeService {
    private HR_API_URL = environment.apiUrls.hr;
    private IT_API_URL = environment.apiUrls.it;
    ...
    getEmployeeById(employeeId: number): Observable<Employee> {
        return this.http.get<Employee>(
            `${this.HR_API_URL}/employees/${employeeId}`
        );
    }

    startOnboarding(employeeId: number): Observable<String> {
        return this.http.post<String>(
            `${this.HR_API_URL}/onboarding/assign/${employeeId}`,
            {}
        );
    }
    ...
    getTasksForEmployee(employeeId: number): Observable<Task[]> {
        return this.http.get<Task[]>(
            `${this.HR_API_URL}/onboarding/employee/${employeeId}`
        );
    }

    completeTask(taskId: number, status: string): Observable<any> {
        return this.http.put(
            `${this.HR_API_URL}/onboarding/status/${taskId}?status=${status}`,
            {}
        );
    }
    ...
    offboardEmployee(employeeId: number): Observable<any> {
        return this.http.post(
            `${this.HR_API_URL}/employees/offboard/${employeeId}`,
            {}
        );
    }
    ...
    createEmail(employeeId: number, data: Partial<Employee>) {
        return this.http.post(
            `${this.IT_API_URL}/itTask/createAcc/${employeeId}`,
            data
        );
    }

    getGovtDocument(employeeId: number): Observable<DocumentMeta> {
        return this.http.get<DocumentMeta>(
            `${this.IT_API_URL}/documents/employee/${employeeId}/document-metadata`
        );
    }

    uploadGovtDocument(employeeId: number, formData: FormData): Observable<any> {
        return this.http.post(
            `${this.IT_API_URL}/documents/upload/${employeeId}`,
            formData
        );
    }
